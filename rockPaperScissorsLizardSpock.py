...
global answerCheckList;
answerCheckList = [item for sublist in acceptAnswers.values() for item in sublist]
...
def get_playerNumber():
	while True:
		try:
			global playerNumber;
			playerNumber = int(input("Enter the Number of Player(s): "))
			if playerNumber == 1:
				# print("Solo-Player")
				return playerNumber
			elif playerNumber == 2:
...
def startGame_multiPlayers():
	...
	while True:
		global p1Opt;
		p1Opt = input("{p1}, Rock, Paper, or Scissors? ".format(p1=p1Name.upper())).lower()
		global p2Opt;
		p2Opt = input("{p2}, Rock, Paper, or Scissors? ".format(p2=p2Name.upper())).lower()
		...
		if p1Opt.lower() in answerCheckList and p2Opt.lower() in answerCheckList:
			...
			if result == "draw":
				...
				print("\n>> {p1} {result1}s, {p2} {result2}s! <<".format(p1=p1Name.upper(),
				                                                         result1=outcomes.get(p1Opt).get(
					                                                         p2Opt), p2=p2Name.upper(),
				                                                         result2=outcomes.get(p2Opt).get(
					                                                         p1Opt)))
			break
		else:
	...
	while True:
		...
		if playerOpt.lower() in answerCheckList:
			...
			if result == "draw":
				...
				print("{playerName} {result}s!".format(playerName=playerName.upper(),
				                                       result=outcomes.get(playerOpt).get(computerOpt)))
			break
		else:
			print("Error: Enter Rock, Paper, or Scissors only!! Try Again!!")
